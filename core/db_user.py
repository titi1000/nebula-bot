...
class DBuser:
    ...
    def db_execute(self, sql, vars:tuple=None):
        try:
            if vars is not None: self.cursor.execute(sql, vars)
    ...
    def is_in_database_user(self, user_id):
        for table in self.tables:
            ...
            if result[0] == True and result[1] is None: self.db_execute(f"INSERT INTO {table}(user_id) VALUES (?)", (user_id,))
    ...
    def is_vip(self, user_id):
        ...
        if result[0]: return bool(result[1][0])
    ...
    def set_grade(self, user_id, grade:str, action:str):
        ...
        if action=="add":
            new_grade = 1
            if new_grade != last_grade: self.db_execute(f"UPDATE grades SET {grade}=? WHERE user_id=?", (new_grade, user_id))
